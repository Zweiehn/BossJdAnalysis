...
## 输出 JSON 字段定义：
{
  ...
  "优先级": "若无需优先级评判则为空，否则按规则返回 优先/观望/谨慎/排除",
  "个人备注": "对该岗位的简要观察、建议或注意事项；如评判了优先级请说明原因"
}
...
class DeepSeekAI(BaseAI):
    ...
    def _build_messages(self, raw_text: str, resume_context: str = "",
                        priority_context: str = "") -> list:
        """构建消息列表"""
    ...
    def analyze_jd(self, raw_text: str, resume_context: str = "",
                    priority_context: str = "") -> dict:
        """
        分析JD文本，返回结构化JSON数据
        ...
        Args:
            ...
            priority_context: 优先级评判规则，空字符串则不评判
        ...
        payload = {
            "model": self.model,
            "messages": self._build_messages(raw_text, resume_context, priority_context),
